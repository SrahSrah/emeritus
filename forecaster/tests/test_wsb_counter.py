...
def test_only_cashtags_count_since_the_first_live_night() -> None:
    """2026-08-31: bare tokens were cut after live night one reported five acronyms
    and zero tickers (EV, GPU, AGI, API, CI). Bare NVDA no longer counts."""
    posts = _posts(("$NVDA to the moon", ""), ("NVDA earnings tomorrow", ""))
    table = count_mentions(posts, stoplist=[])
    assert table["NVDA"]["count"] == 1
    assert table["NVDA"]["post_urls"] == ["https://r.test/0"]


def test_the_measured_live_false_positives_no_longer_match() -> None:
    """The exact tokens the 2026-08-31 digest reported, as bare text: none may count."""
    posts = _posts(
        ("EV plays and the GPU shortage", "AGI soon; the API is down, CI is red"),
    )
...
def test_a_stoplisted_token_yields_no_entry_even_as_a_cashtag() -> None:
