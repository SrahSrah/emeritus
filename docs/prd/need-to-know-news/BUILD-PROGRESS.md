...
| 47 | Importance judgment: gate → judge → suppress-when-unsure | **done** | `2f70313` | 566 tests (+8). Gate reads the observed counts (corroboration_observed now carries min_sources, so FR-41 computes gate-passes from the trace alone). Unrecognisable verdict = uncertainty = suppress. Judgment failure = named abstention with the unassessed count; already-judged deliveries stand. Bar lists proven to reach the prompt from config. v4 metric conditions verified to still hold under the bar. |
| 48 | Pulse line: quiet nights and abstentions | **done** | `ec2b6d3` | 570 tests (+4 new, 4 v4-contract tests updated to the v5 contract by design). The pulse tally is itself traced as an observation so FR-11's support check has a payload to match — counts declared in checkable_fields, provenance-verified in tests. Reframe-only proven under the date rule. |
| 49 | Cross-beat deferral, one-way | **done** | `227d193` | 575 tests (+5). CROSS_BEAT_DEFER policy constant in the synthesizer (dedup machinery, not beat registration — the dummy-beat seam contract holds). Cross-beat neighbours stamped with their source beat so deferrals name their cover; the deferral record replaces the plain suppression record. FR-27 veto and invariant 2 proven intact. Bonus: FR-30 quarantined the test's own first fixture for an ungrounded figure — the guardrail caught its author. |
| 50 | Bar-phase metric (d)–(f) + gate-pass count | **done** | — | 581 tests (+6). (d) also cross-checks the pulse line's declared counts against the outcome tally; (e) includes the run-level invariant-2 check (a watchlist night can never defer at all); (f) is report-only by construction. Verified on real data/runs: (a)–(c) pass over 4 nights, (d)/(e) n/a pre-v5, gate-pass printed as 'n/a (pre-v5 trace)' — history not recomputed under the new config. |
